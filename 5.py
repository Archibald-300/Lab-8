...
class Gun():
    ...
    def __init__(self, coord=[30, SCREEN_SIZE[1] // 2], angle = 0, maximum_power = 80, minimum_power = 10, gun_color = BLACK):
        '''
        Seting of coordinates, direction, minimumimum and maximumimum power and color of the gun
        '''
        self.coord = coord
        self.angle = angle
...
class Ball():
    ...
    def draw(self, screen):
        '''
        Draws the ball on appropriate surfaceace.
        '''
...
class Target():
    ...
    def __init__(self, coord=None, color=None, ball_radius=30):
        '''
        Constructor method. Sets coordinate, color and ball_radiusius of the target.
        '''
        if coord == None:
            coord = [randint(ball_radius, SCREEN_SIZE[0] - ball_radius), randint(ball_radius, SCREEN_SIZE[1] - ball_radius)]
        self.coord = coord
        self.ball_radius = ball_radius
...
class ScoreTable():
    ...
    def draw(self, screen):
        ...
        score_surface = []
        score_surface.append(self.font.render("Tergets hitted: {}".format(self.smashed_targets), True, BLACK))
        score_surface.append(self.font.render("Balls used: {}".format(self.number_of_used_balls), True, BLACK))
...
class editor():
    ...
    def new_mission(self):
        ...
        if self.score_table.score() < 0:
            ...
            for i in range(self.number_targets):
                self.targets.append(Target(ball_radius = randint(max(1, 30 - 2 * self.score_table.score()), 30 - self.score_table.score()))) 
    ...
    def strike(self):
        ...
        #making list with targets for deletting
